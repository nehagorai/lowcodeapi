...
export function Explorer({ 
    ...
    const { provider, providersall, providers, id, tab, category, categories, categoriesall, focus, path, paths } = router.query;
    ...
    useEffect(() => {
        ...
        ;(async () => {
            ...
            try {
                ...
                if (!definition.details.service_url && window) {
...
export const getStaticProps = async (context) => {
    ...
    const { provider, ...params } = context.params;

    try {
        const filter = params.paths ? [...params.paths] : [];
        let qs = 'extra_key=1';
